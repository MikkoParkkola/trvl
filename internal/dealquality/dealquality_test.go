...
func TestQueryFilters(t *testing.T) {
	...
	// trvl#552: relative dates -- see TestDedup comment above. The season is
	// derived from the date rather than written as a "Q2" literal: a hardcoded
	// season is the same class of time-bomb, failing the moment real time
	// leaves that quarter.
	d1 := recentDate(1)
	d2 := recentDate(2)
	season := SeasonOf(d1)
...
func TestStoreLoad(t *testing.T) {
	...
	day := recentDate(1)
	season := SeasonOf(day)
	_ = store.Append(Sample{Route: "HEL-BCN", Season: season, Date: day, Price: 150, Kind: "flight"})
