...
func TestAddSeparatesThresholdsAndCollapsesRepeats(t *testing.T) {
	...
	id1, _, err := s.Add(routeWatch509(200))
	if err != nil {
		t.Fatalf("add first threshold: %v", err)
	}
	id2, _, err := s.Add(routeWatch509(120))
	if err != nil {
	...
	repeat, _, err := s.Add(routeWatch509(200))
	if err != nil {
...
func TestCheckAllBoundedPollsEachTargetOnce(t *testing.T) {
	...
	for _, below := range []float64{200, 150, 120} {
		if _, _, err := s.Add(routeWatch509(below)); err != nil {
...
func TestThresholdsAlertIndependently(t *testing.T) {
	s := NewStore(t.TempDir())
	idHigh, _, err := s.Add(routeWatch509(200)) // 150 <= 200 -> should fire
	if err != nil {
		t.Fatalf("add high threshold: %v", err)
	}
	idLow, _, err := s.Add(routeWatch509(120)) // 150 > 120 -> must not fire
	if err != nil {
