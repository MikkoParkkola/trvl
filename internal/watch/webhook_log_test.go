...
//
// Merge note (release/1.21.0 -> main): this input no longer reaches the
// create-request line. main added a round-22 scheme gate that url.Parse-es the
// webhook up front and rejects anything that is not http/https, so a URL this
// malformed is refused strictly EARLIER than the path this test was written
// for. The leak vector is therefore closed twice over.
//
// The assertion is widened to "some diagnostic line was emitted", not dropped:
// the property under test is that the secret never reaches a log record, and
// that is asserted unconditionally below. A test pinned to the exact message of
// whichever guard happens to fire first would break on any future reordering
...
func TestFireWebhookCreateRequestFailureDoesNotLogTheSecret(t *testing.T) {
	...
	if !strings.Contains(out, "webhook: create request") &&
		!strings.Contains(out, "webhook: rejecting unsupported URL scheme") {
		t.Fatalf("expected a diagnostic line for a malformed webhook URL, got: %s", out)
	}
	if strings.Contains(out, secret) {
