...
func TestConcurrentAddsFromSeparateStoresBothSurvive(t *testing.T) {
	...
	go func() {
		_, _, err := a.Add(Watch{Type: "flight", Origin: "AMS", Destination: "VLC", BelowPrice: 100, Currency: "EUR"})
		aDone <- err
	}()
	...
	go func() {
		close(bStarted)
		_, _, err := b.Add(Watch{Type: "flight", Origin: "HEL", Destination: "BCN", BelowPrice: 200, Currency: "EUR"})
		bDone <- err
	}()
...
func TestCheckRoundDoesNotRevertConcurrentToolCall(t *testing.T) {
	...
	seed := NewStore(dir)
	id, _, err := seed.Add(Watch{Type: "flight", Origin: "AMS", Destination: "VLC", BelowPrice: 500, Currency: "EUR", LastPrice: 480})
	if err != nil {
