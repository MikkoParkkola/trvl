...
func (s *Store) backupPathsLocked(paths []string) (string, error) {
	...
	for _, path := range paths {
		// #nosec G304 -- paths contains only Store's fixed legacy/database files
		// under its configured owner-only directory.
...
func writeNewBackup(path, stamp string, data []byte) (string, error) {
	...
	for i := 0; i < 64; i++ {
		...
		// #nosec G304 -- dst is generated beside a fixed Store path and opened
		// O_EXCL; no request-controlled path component is used.
...
func verifyBackup(dst string, want []byte) error {
	// #nosec G304 -- dst is the backup path just generated by writeNewBackup in
	// the Store state directory.
