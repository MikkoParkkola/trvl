...
func WriteICS(t *trips.Trip) string {
	...
	if t.Workspace != nil {
		for i, day := range t.Workspace.Days {
			writeDayEvent(&b, t, i, day, now)
		}
	}
...
// writeDayEvent renders one all-day VEVENT for a day-graph DayPlan. Days
// without a parseable date are skipped so the calendar stays valid. The event
// title comes from DayPlan.Title, falling back to a place-count summary.
func writeDayEvent(b *strings.Builder, t *trips.Trip, idx int, day trips.DayPlan, now time.Time) {
	if !startDateOnly(day.Date) {
		return
	}
	writeLine(b, "BEGIN:VEVENT")
	writeLine(b, "UID:"+makeDayUID(t, idx, day))
	writeLine(b, "DTSTAMP:"+now.Format("20060102T150405Z"))

	writeLine(b, "DTSTART;VALUE=DATE:"+strings.ReplaceAll(day.Date[:10], "-", ""))
	if d, err := models.ParseDate(day.Date[:10]); err == nil {
		// All-day end date is exclusive in RFC 5545 — use the day after.
		writeLine(b, "DTEND;VALUE=DATE:"+d.AddDate(0, 0, 1).Format("20060102"))
	}

	writeLine(b, "SUMMARY:"+escapeText(dayEventSummary(day)))
	if day.EstimatedRouteMinutes > 0 {
		writeLine(b, "DESCRIPTION:"+escapeText(
			fmt.Sprintf("Day plan: %d place(s), ~%d min on the move. Generated by trvl.",
				len(day.PlaceIDs), day.EstimatedRouteMinutes)))
	}
	writeLine(b, "STATUS:CONFIRMED")
	writeLine(b, "END:VEVENT")
}

// dayEventSummary returns the title for a day-graph event.
func dayEventSummary(day trips.DayPlan) string {
	if day.Title != "" {
		return "🗺️ " + day.Title
	}
	return fmt.Sprintf("🗺️ Day plan (%d places)", len(day.PlaceIDs))
}

// makeDayUID returns a stable globally-unique identifier for a day-graph event.
func makeDayUID(t *trips.Trip, idx int, day trips.DayPlan) string {
	h := sha256.New()
	_, _ = fmt.Fprintf(h, "%s|day|%d|%s|%s", t.ID, idx, day.ID, day.Date)
	return fmt.Sprintf("trvl-%s-day-%d-%s@trvl.local",
