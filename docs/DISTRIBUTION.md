...
| Official MCP Registry | Live | Release workflow stamps `server.json` from the pushed tag and publishes the matching OCI package (`ghcr.io/mikkoparkkola/trvl:<version>`) through `mcp-publisher`. The checked-in manifest is aligned to v1.9.2 for this release. |
