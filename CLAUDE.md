...
- Go 1.26.6 · MCP 2025-11-25 · single binary · 24 transport providers (+ flight & hotel sources below)
...
- Current release: v1.21.6, published 2026-09-10 from `main` to GitHub Releases, Homebrew, npm, GHCR, the Go module proxy, and the official MCP Registry.
- v1.21.6 publishes OAuth Protected Resource Metadata (RFC 9728), rotates the Wizz Air API version to 29.15.1 via homepage-config discovery, declares per-package environment variables in `server.json`, and ships grpc 1.83.2.
...
| **`GOTOOLCHAIN=go1.26.6` pinning via Makefile** | CI reproducibility; host `go` on PATH may be older | Run raw `go build/test` without the prefix on older hosts |
...
GitHub Actions (`.github/workflows/ci.yaml`): build, vet, staticcheck, govulncheck, test with race detector, coverage threshold (80%). Runs on ubuntu + windows, Go 1.26.6.

Make targets pin `GOTOOLCHAIN=go1.26.6` so local build/test entrypoints match CI even when the host `go` on `PATH` is older. For raw `go ...` commands on such hosts, prefix `GOTOOLCHAIN=go1.26.6`.
