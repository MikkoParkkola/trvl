...
func renderMultidestTable(out *os.File, bundles []multidest.Bundle) {
	if len(bundles) == 0 {
		_, _ = fmt.Fprintln(out, "No bundles to rank — check input orderings.")
		return
	}
	_, _ = fmt.Fprintf(out, "Top %d multi-destination bundles:\n", len(bundles))
	_, _ = fmt.Fprintf(out, "  %-4s  %-28s %10s %10s %10s  %s\n", "Rank", "Cities", "Flights", "Hotels", "Total", "Notes")
