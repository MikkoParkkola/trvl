...
func hotelPricesOutputSchema() interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"booking_readiness":                 schemaStringDesc("Composite booking-readiness verdict (MIK-6232) for the selected lowest-price seller, one of: ready, caution, unverified. 'ready' requires that same seller to have a verified price AND stable link AND confirmed identity AND known refundability; signals are never borrowed from another seller. Treat anything below ready as 'verify before booking' and tell the user why via booking_readiness_reasons."),
			...
			"booking_readiness_ceiling":         schemaStringDesc("Present only when the selected lowest-price seller cannot reach 'ready' because it did not supply cancellation terms. Seller refundability is carried when the provider exposes it, so this ceiling is conditional rather than permanent. IMPORTANT: it explains missing evidence only; booking_readiness_reasons may still report actionable findings such as an expiring link or an unverified price. Use hotel_rooms for richer room-level evidence."),
...
func hotelRoomsOutputSchema() interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"booking_readiness":         schemaStringDesc("Composite booking-readiness verdict (MIK-6232), one of: ready, caution, unverified. Reachable to 'ready' when one concrete room offer carries every required signal; signals are never combined across rooms. Anything below ready means verify before booking."),
