...
func hotelPricesOutputSchema() interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"booking_readiness_ceiling":         schemaStringDesc("Present only when this endpoint cannot reach 'ready' whatever the property. It carries no cancellation terms, so refundability is unobtainable and 'caution' is the best it can report. When present, do NOT read the verdict as a finding about this hotel: use hotel_rooms for a room-level verdict that can reach 'ready'."),
