...
func hotelPricesOutputSchema() interface{} {
	return map[string]interface{}{
		...
		"properties": map[string]interface{}{
			...
			"booking_readiness_ceiling":         schemaStringDesc("Present only when this endpoint cannot reach 'ready' whatever the property. It carries no cancellation terms, so refundability is unobtainable and 'caution' is the best it can report. IMPORTANT: this explains the ceiling ONLY. It does not mean the verdict is free of findings about this hotel: booking_readiness_reasons may still report a genuine problem such as an expiring link or an unverified price, and those remain actionable. Read booking_readiness_ceiling_reasons for the signals that are structurally unobtainable, and booking_readiness_reasons for everything else. Use hotel_rooms for a room-level verdict that can reach 'ready'."),
